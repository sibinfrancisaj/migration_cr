...
const mockFlagMessage = jest.fn();

jest.mock('@abroad-matrimony/messaging', () => {
  class MessageNotFoundError extends Error {
    constructor() { super('MESSAGE_NOT_FOUND'); this.name = 'MessageNotFoundError'; }
  }
  class AlreadyFlaggedError extends Error {
    constructor() { super('ALREADY_FLAGGED'); this.name = 'AlreadyFlaggedError'; }
  }
  class FlagSelfError extends Error {
    constructor() { super('FLAG_SELF'); this.name = 'FlagSelfError'; }
  }
  return {
    ...
    flagMessage:             (...a: any[]) => mockFlagMessage(...a),
...
jest.mock('@abroad-matrimony/auth', () => ({
  requireAuth: (req: any, res: any, next: any) => {
    ...
    req.user      = { id: 'user-reporter', role: 'USER', deviceId: 'device-1' };
...
describe('POST /api/v1/messages/:msgId/flag', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    requireAuthBehavior = 'pass';
    mockFlagMessage.mockResolvedValue(FLAG_RESULT);
  });

  it('returns 201 with flag data on success', async () => {
    const res = await request(app)
      .post(`/api/v1/messages/${MSG_ID}/flag`)
      .send(VALID_BODY);

    expect(res.status).toBe(201);
    expect(res.body.success).toBe(true);
    expect(res.body.data.id).toBe('flag-uuid-1');
    expect(mockFlagMessage).toHaveBeenCalledWith('user-reporter', MSG_ID, 'SPAM', undefined);
  });

  it('passes optional description to service', async () => {
    await request(app)
      .post(`/api/v1/messages/${MSG_ID}/flag`)
      .send({ reason: 'HARASSMENT', description: 'Offensive language' });

    expect(mockFlagMessage).toHaveBeenCalledWith(
      'user-reporter',
      MSG_ID,
      'HARASSMENT',
      'Offensive language',
  ...
  it('returns 400 when msgId is not a UUID', async () => {
    const res = await request(app)
      .post('/api/v1/messages/not-a-uuid/flag')
      .send(VALID_BODY);
  ...
  it('returns 404 when message is not found', async () => {
    const mock = jest.requireMock('@abroad-matrimony/messaging') as {
      MessageNotFoundError: new () => Error;
    };
    mockFlagMessage.mockRejectedValue(new mock.MessageNotFoundError());
  ...
  it('returns 409 when message is already flagged by this user', async () => {
    const mock = jest.requireMock('@abroad-matrimony/messaging') as {
      AlreadyFlaggedError: new () => Error;
    };
    mockFlagMessage.mockRejectedValue(new mock.AlreadyFlaggedError());
  ...
  it('returns 409 when user tries to flag their own message', async () => {
    const mock = jest.requireMock('@abroad-matrimony/messaging') as {
      FlagSelfError: new () => Error;
    };
    mockFlagMessage.mockRejectedValue(new mock.FlagSelfError());
  ...
  it('returns 500 on unexpected service error', async () => {
    mockFlagMessage.mockRejectedValue(new Error('DB down'));
